    ...
    >
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16 sm:mb-20">
          <h2
            id="features-heading"
            className="text-3xl sm:text-4xl lg:text-5xl font-bold text-[#4a1f38] mb-4"
          >
            Powerful Features
          </h2>
          <p className="text-lg text-[#7a4f5a] max-w-2xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-10">
          {features.map((feature, index) => (
            <div
              key={index}
              className="group p-6 rounded-2xl border border-[#ffc2cc] bg-white hover:border-[#c084a0] hover:shadow-md transition-all duration-200 focus-within:ring-2 focus-within:ring-[#c084a0] focus-within:ring-offset-2"
              tabIndex={0}
            >
              <div className="flex items-start space-x-4">
                <div
                  className="flex-shrink-0 w-12 h-12 rounded-xl bg-[#fce8e0] text-[#9b5c7a] flex items-center justify-center group-hover:scale-110 transition-transform duration-200"
                  aria-hidden="true"
                >
                  {feature.icon}
                </div>
                <div className="flex-1 min-w-0">
                  <h3 className="text-xl font-semibold text-[#4a1f38] mb-2">{feature.title}</h3>
                  <p className="text-[#7a4f5a] leading-relaxed">{feature.description}</p>
                </div>
              </div>
            </div>
          ))}
